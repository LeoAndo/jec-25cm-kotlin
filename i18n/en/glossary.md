...
- 担当：翻訳は Claude Code の翻訳用subagent（Opus 5.5）10体。照合は、翻訳した担当とは別のモデルの subagent（Sonnet 5）10体。照合の担当は、翻訳スキル・用語集・日本語のページを自分で読み、対訳を並べて照合した。照合のあとで訳を変えた文は、別の subagent（Sonnet 5）が照合し直した（PR #69 の決まり。#57・#58 の分も、オーナーの判断でさかのぼって照合し直した）。
- 照合の全文：各PRのコメント（PR #66・#70・#73・#74・#76）と、#62 のコメント。
