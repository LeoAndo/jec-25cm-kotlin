...
## IntelliJ IDEAの画面に出る日本語（K01。日本語のまま残し、英語の表示名をかっこで添える）

K01の教科書は、日本語表示の IntelliJ IDEA の画面の言葉を書いている。画面の言葉は訳さずに残し、うしろに英語で表示したときの名前をかっこで添える（例：場所 (Location)。下の「書き方の決まり」）。表示名の末尾のコロン（`Location:` の `:`）は付けない。この表の表示名は、IntelliJ IDEA 2026.3 EAP（IU-263.5153.40）の本体とプラグインの `messages/*.properties` で、日本語の言語パック（`localization-ja`）と同じキーの値を読んで確かめた（2026-09-25、#99）。K01の本文ですでに訳してある、ほかの画面の言葉は、カタログの訳を見る。

| 画面の言葉 | かっこに添える表示名 | 場所 |
| --- | --- | --- |
| 場所 | Location | 新規プロジェクトの画面。K01の表の `<td>場所</td>` は、準備ガイドなどの表の見出し `<th>場所</th>`（Location）と訳し分けるため、K01のカタログの `overrides` に置く |
| 名前 | Name | 新規プロジェクトの画面。ただしK01の表の `<td>名前</td>` は、引数の表（STEP 15の「先に書くもの」の列）にも同じ形で2つあり、`overrides` でも訳し分けられない。そのため表の訳は Name のままにし、日本語を添えない（#99、オーナー判断） |
| ビルドシステム | Build system | 新規プロジェクトの画面 |
| サンプルコードの追加 | Add sample code | 新規プロジェクトの画面 |
| コンパクトなプロジェクト構造を使用する | Use compact project structure | 新規プロジェクトの画面 |
...
### K01の新規プロジェクトの画面の項目名と「使うもの」の見出し（#99）

- 原文の版：`b41ece3`（`docs/hello-kotlin/index.html` のSHA256 `bf9971bb4978764607bb7d8686afc6cd96b5068ff1ad0600879b85b734b55ae2`）
- 照合範囲：K01の4文（`828d4cec1f3c` 場所の `overrides`、`9b1574eb2544` ビルドシステム・`10c3e349397c` サンプルコードの追加・`50ce8c94f0d8` コンパクトなプロジェクト構造を使用する の既定訳）と、上の「IntelliJ IDEAの画面に出る日本語」の表示名。
- 担当：翻訳は Claude Code / Opus 5.5。照合は別のモデルの subagent（Sonnet 5。5言語をまとめて1回）。訳文だけを渡してファイルを読ませずに訳し戻しを確定させ、そのあとで翻訳skill・用語集・日本語のページ・カタログの差分・IntelliJ IDEA の言語パックを自分で読ませて比べた（2026-09-25）。
- 照合の全文：PR #101 のコメント。要修正の指摘はなし（2段階目の8件とも対応不要）。照合のあとで変えた訳はない。

| ID | 原文 | 指摘 | 対応 |
| --- | --- | --- | --- |
| `10c3e349397c` | サンプルコードの追加 | 1段階目：Add sample code だけ動詞句で、表のほかの行（Location・Build system）とそろわない | 対応不要。英語表示の IntelliJ IDEA の表示名そのもの。2段階目も同じ判定 |
| `7ec26292414b` | 名前 | （変えていない文）`<td>名前</td>` が引数の表（STEP 15）と訳し分けられない | Name のまま（オーナー判断）。2段階目：判断は妥当 |
