...
- **教科書と教員用ガイドに、振り返りの言い方が残っていないかを見る。** 確認欄（`.check`）や「最後の3分」に「気づいたことを1つメモします」「〜を言えた」「〜を説明できた」、教員用ガイドの「終了時の確認」「到達度の確認」に「〜と言える」「〜まで言えたら到達」があれば、**対応が必要**な指摘とする（AGENTS.md「受講生像と、教材の書き方」）。直し方は、画面で確かめられる事実（出た行、赤くなった行、マウスを重ねて出た型）に置き換えること。
  ```sh
  grep -nE '言える|言えた|言えれば|説明できた|説明できれば|気づいたことを1つメモ' docs/hello-kotlin/index.html teacher/hello-kotlin/index.html
  ```
