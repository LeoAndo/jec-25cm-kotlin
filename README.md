...
**いまは英語だけが `config/i18n.json` の `distribute: true` です**（2026-09-25。全ページの翻訳と、別のAIによる照合が済んだ。照合の記録は `i18n/en/glossary.md`）。ほかの6言語は `false` で、翻訳が途中のため、配布対象にすると公開ゲートで止まります。初回翻訳と、別のAIによる独立した照合まで終わった言語から、その言語だけ `distribute` を `true` に上げてください。配布対象にした言語は、ZIPを作るときに `docs/<言語>/` へ生成されます。対象が0言語のあいだは、言語の入口や翻訳HTMLは追加せず、日本語だけの構成で配布します。
