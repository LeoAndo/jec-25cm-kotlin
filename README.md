...
1. **1単元で導入する新概念は1つまで。** 「新概念」は、それまでの単元に1度も出ていない考え方やAPIのうち、**JavaにもSwiftにもない**ものを指します。実在するかは `git grep` で確認します。2つ以上必要に見えるときは、単元を分けるか、片方を後の単元へ回します。
   - 受講生はJavaとSwiftを学習済みなので、`if` や `for` のように**書き方だけが違うもの**は新概念に数えません。K03が関数・条件式・繰り返しを、K05が `data class`・スマートキャスト・拡張関数を1コマでまとめて扱えるのは、このためです。新概念に数えるのは、委譲プロパティ（`by lazy` の `by`）のように、**JavaにもSwiftにも対応物がなく、知っている言語の知識では説明が付かないもの**です。数え方は下の「純Kotlin系（K01〜K06）での新概念の数え方」にまとめています。
...
## 純Kotlin系（K01〜K06）での新概念の数え方

**「新概念」は、JavaにもSwiftにもない考え方を指します。書き方だけが違うものは数えません。** 受講生はJavaとSwiftを書けるので、どちらかに対応物がある項目は「あの書き方が、Kotlinではこうなる」と見せれば伝わります。1コマに入るのは、覚えることの数ではなく、比較で片付かないものの数です。**純Kotlin系（K01〜K06）が1コマで複数の文法項目を扱うのは、このためです**（「15コマ計画」の `内容` 列）。

各項目のJava／Swiftでの対応物は次のとおりです。教科書では、この対応物を比較ブロックで見せます（「授業用教科書の基本方針」2・4）。

| コマ | Kotlinで扱う項目 | Javaでの対応物 | Swiftでの対応物 |
| --- | --- | --- | --- |
| K01 | `val` / `var`、型推論 | `final` 付きの変数／通常の変数、`var`（Java 10以降） | `let` / `var`、型推論 |
| K01 | セミコロン不要 | 文末に `;` が必要 | 同じく不要 |
| K02 | Null許容型 `?`、安全呼び出し `?.` | **対応物なし**（`Optional` は型システムの外側の約束） | オプショナル `String?`、オプショナルチェイン `?.` |
| K03 | 関数 `fun`、条件式 `if`、繰り返し `for (x in list)` | メソッド、`if`、拡張for文 | `func`、`if`、`for x in list` |
| K04 | クラス、プライマリコンストラクタ | コンストラクタ（クラス本体の中に書く） | イニシャライザ、structのメンバーワイズイニシャライザ |
| K04 | プロパティとアクセサ（`get()` / `set()` / `field`） | getter / setterを自分で書く | computed property、`willSet` / `didSet` |
| K05 | `data class` | `record`（Java 16以降） | `struct`（`Equatable` などの自動合成） |
| K05 | スマートキャスト | `instanceof` のパターンマッチ（Java 16以降） | `if let`、`as?` |
| K05 | 拡張関数 | **対応物なし** | `extension` |
| K06 | `map` などのコレクション操作 | Stream APIの `map` | `map` |
| K06 | `by lazy`（委譲プロパティ） | **対応物なし** | **対応物なし**（`lazy var` は結果が似るだけで、委譲の仕組みはない） |

- **両方が「対応物なし」なのは `by lazy`（委譲プロパティ）だけです。** K01〜K06でこの数え方に引っかかる新概念は、K06のこの1つです。だからK05は、`data class`・スマートキャスト・拡張関数の3つを1コマで扱っても「1単元1新概念」に反しません。3つとも、受講生が知っている書き方の言い換えとして説明できます。
- **片方だけ「対応物なし」の項目は、正直にそう書きます。** Null許容型（K02）と拡張関数（K05）はJavaにありません。Javaだけを知っている人には新概念ですが、この授業の受講生はSwiftで同じ考え方を使っています。教科書では、Javaとの比較で「Javaではこうは書けない」を見せ、Swiftとの比較で「知っているあれと同じ」を見せます。
