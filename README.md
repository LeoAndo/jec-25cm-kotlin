...
学生用ZIPには `docs` 一式と、展開済みの完成プロジェクト（`samples`）、開き方・版情報を収録します。**日本語・英語・中国語（簡体）の3言語で配布します**（英語は2026-09-25に `distribute: true` にした。#56。中国語（簡体）は、2026-09-28の授業開始に合わせた初版のために、同じ日に加えた。#126）。展開してできたフォルダの中身は、言語を選ぶ入口の `index.html` と `docs` / `samples` / `はじめに.txt` / `VERSION.json` の5つです。英語の教科書は `docs/en/`、中国語（簡体）の教科書は `docs/zh-Hans/` に入り、`はじめに.txt` にも両言語の案内が付きます。ほかの7言語は `distribute: false` なので入りません（「多言語展開」）。翻訳を終えた言語を `distribute: true` にすると、その言語も入口と `はじめに.txt` に加わります。`teacher` フォルダと、教員が確認に使うひな形（`Panda2KotlinEmptyViewsActivity` / `Quail4KotlinEmptyViewsActivity`）は収録しません。GitHubが自動で表示する **Source code (zip)** はリポジトリ全体のため、学生用ZIPには使いません。なお、このリポジトリ自体はPublicなので、教員用ファイルもGitHub上では閲覧できます。
...
2. **配布準備の翻訳PR（`distribute: true` の言語の差分翻訳）を開いてから公開するまでは、`docs/` を触るPRをマージしません。** 訳しているあいだに日本語が変わると、公開ゲートが公開の直前に止まるためです。配布していない言語（`distribute: false`）の翻訳PRが開いていても、`docs/` を触るPRのマージは止めません（学生向けの公開がないためです。日本語が先に変わったら、翻訳PRの側が最新のmainを取り込みます）。訳した文が50文以上なら、訳したのとは別のAIが訳文を原文と照合します（ミャンマー語だけは、日本語へ訳し戻してから比べます）。50文未満なら照合せず、翻訳の担当が見直します。検査と配布ZIPの確認を済ませて翻訳PRをマージします。日本語の変更が先に入った場合は、最新のmainで差分を訳し直します。
...
日本語で書いた教科書（`docs/`）を、配布前に学生の母国語へ展開します。対象は、日本語のほかに次の9言語です。台湾華語・スペイン語・アラビア語・モンゴル語は、2026-09の学生アンケート（母国語の確認）で加えました。

**初版は、日本語・英語・中国語（簡体）の3言語で配布します**（2026-09-25 オーナー決定）。2026-09-28の授業開始に間に合わせるためです。ほかの7言語は、後期の授業が始まってから、1言語ずつ段階的に訳して配布対象に加えます（[#116](https://github.com/LeoAndo/jec-25cm-kotlin/issues/116)）。

**いまは英語と中国語（簡体）が `config/i18n.json` の `distribute: true` です**（英語は2026-09-25、#56。照合の記録は `i18n/en/glossary.md`。中国語（簡体）は2026-09-25、#126。照合の記録はそのPRのコメント）。どちらも全ページの翻訳と、別のAIによる照合が済んでいます。ほかの7言語は `false` です（翻訳が途中か、まだ始まっていない）。未翻訳のある言語を配布対象にすると、公開ゲートで止まります。全文を訳し終え、別のAIがその言語の全文を1回照合した言語から、その言語だけ `distribute` を `true` に上げてください。**照合は、学生の手に渡る前のこの1回を中心にします**（2026-09-25 オーナー決定、#100）。配布していない言語の翻訳PRでは照合しません。くわしくは [AGENTS.md](AGENTS.md) §12 と [翻訳用skill](skills/translate-teaching-materials/SKILL.md) の「別のモデルによる照合」にあります。配布対象にした言語は、ZIPを作るときに `docs/<言語>/` へ生成されます。対象が0言語のあいだは、言語の入口や翻訳HTMLは追加せず、日本語だけの構成で配布します。
...
| アラビア語 | `ar` | `false` | 右から左に書く（`dir: "rtl"`） |
| モンゴル語 | `mn` | `false` | キリル文字（モンゴル国の表記）。横書き |

モンゴル語は、学生本人に読む文字を確かめて、モンゴル国のキリル文字（`mn`）で加えました（[#92](https://github.com/LeoAndo/jec-25cm-kotlin/issues/92)）。伝統的モンゴル文字（縦書き）には対応しません。同じ学生が受けているハイブリッドアプリ開発技法でも同じ扱いです（[jec-25cm-hybrid-app #40](https://github.com/LeoAndo/jec-25cm-hybrid-app/issues/40)）。アンケートで挙がらなかった言語も、未回答の学生がいるので外しません。
...
- **翻訳の対象にならない文字の全角の記号は、漢字を使わない言語のページではASCIIの記号にして出します**（[#89](https://github.com/LeoAndo/jec-25cm-kotlin/issues/89)）。かな・漢字を含まない「（Android 12）」「STOP・RESET」「K01：HelloKotlin」のような文字は翻訳の単位にならず、どの言語でも日本語版のまま出るので、ページを作るときに `(Android 12)`・`STOP · RESET`・`K01: HelloKotlin` に置き換えます（範囲の `〜` は en dash `–`）。日本語のページと、漢字を使う言語（中国語・広東語・台湾華語）のページは変わりません。訳文の中の全角の記号は、日本語を残す文を除いて `check` が落とします。
- **アラビア語は右から左に書きます**（[#78](https://github.com/LeoAndo/jec-25cm-kotlin/issues/78)）。`config/i18n.json` で `"dir": "rtl"` にした言語は、翻訳ページの `<html>` に `dir="rtl"` が付き、未翻訳の日本語には `dir="ltr"` が付きます。コード・キー・英字の画面の名前は、右から左のページでも左から右のまま出ます。翻訳の単位を含まない「変更前 → 変更後」の並び（A02 の `<code>9</code> → <code>4</code>` など）も、ページを作るときに `<span dir="ltr">` で包み、左から右に並べます（[#195](https://github.com/LeoAndo/jec-25cm-kotlin/issues/195)）。左から右の言語のページの本文のHTMLは変わりません（言語の切り替えと、言語を選ぶ入口のリンクには、どの言語にも `dir` が付きます。見た目は変わりません）。そのため、教科書のCSS（`docs/assets/textbook.css`）には左右を決め打ちした指定（`border-left`・`padding-left`・`text-align: left` など）を書かず、論理プロパティ（`border-inline-start`・`padding-inline-start`・`text-align: start` など）で書きます。
