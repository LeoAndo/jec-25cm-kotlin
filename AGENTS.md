...
- **Codex を手動で呼ぶときは、PRの会話のコメントの1行目に `@codex review` とだけ書く。** #202 では、この形で呼ぶとレビューが動いた。文の中で `@codex review` に触れただけのコメントには、インラインの返信でもPRの会話でも、コードとして囲んでいても Codex が反応し、「To use Codex here, create a Codex account and connect to github」と返した。Codex の説明では、レビュー以外の頼みごとには Codex のアカウントと GitHub の接続が要る。Codex を呼ぶとき以外は、コメントに `@codex` と書かない。
...
- **#202 では、CodeRabbit は Draft を外してから約10分でレビューを投稿した（指摘2件）。** レビュー本文には「この回で枠を使い切った。いまの枠は1時間に1件」と出た。修正の push のあとの2回目のレビューは、枠を超えた分として従量課金の扱いで行われ、コメントに「Usage-based review receipt」（Mode: Continue automatically、Waived: $0.25、charged $0.00）が付いた。免除された理由は未確認（2026-09-24 の表示では、オンデマンドレビューは16日間は無料だった）。枠の中のレビューは無料で、枠を超えた分は1ファイル $0.25 の設定になっている。上限に達しても「Review paused」で止まらずにレビューが続くので、この設定を変えるかどうかはオーナーが決める。
