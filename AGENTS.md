...
- **`true` に上げてよいのは、次の2つが両方終わった言語だけ。**
  ...
  2. 翻訳したのとは別のAIが、新しく訳した文を日本語へ逆翻訳し、原文と照合し終えている（下の手順4）。**照合は、翻訳と同じセッション・同じPRの中のsubagentが行ってもよい**（オーナーの判断、2026-09-24）。そのときも、訳したsubagentとは別のsubagent（できれば別のモデル）にする。自分で訳して自分で照合しない。
