...
- Codex の自動レビューは、末尾の「Review guidelines」の節に従って指摘する。指摘の基準を変えるときは、その節を直す。
...
| Codex | ChatGPT Pro に含まれる。2026-09-26 に自動レビューを有効にした | 新しいPRが開かれたときにレビューする。`@codex review` でも呼べる。GitHub では P0・P1 だけを指摘し、末尾の「Review guidelines」に従う。Draft と修正のpush後の動きは未実測 |
...
- **Codex は、2026-09-26 に自動レビューを有効にした。** 最初の実測は、#201 を閉じるPRで取る。
...
Codex の自動レビューは、この節で指摘するかどうかを決める（見出しの文字列を変えない）。コメントは日本語で書く。前提は「受講生像と、教材の書き方」・§10・§11 にある。
